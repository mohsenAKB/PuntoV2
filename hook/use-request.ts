...
export type OnRequestFailed<R = any, D = any> = (
  err: AxiosError<R, D>,
...
const useRequest = <R = any, D = any>(
  ...
  const executeRequest = useCallback(
    async (
      ...
      try {
        ...
        const response: AxiosResponse<R> = await requestInstance.request(
          finalConfig
        );
  ...
  const extractErrorData = (
    error: AxiosError<IList<undefined>>
  ): IList<undefined> | undefined => {
